...
class ForceInputDialog(QtWidgets.QDialog):
    ...
    def changed(self):
        self.buttons.button(QtWidgets.QDialogButtonBox.Ok).setEnabled(
            bool(self.input.text())
        )

    @classmethod
    def get_text(
        cls, parent: QtWidgets.QWidget, title: str, label: str, text: str = ""
    ) -> "ForceInputDialog":
        obj = cls(parent)
...
class StrFilterRow(FilterRow):
    ...
    def get_state(self) -> tuple:
        ...
        query_line = self.filter_query_line.text().translate(
            str.maketrans("", "", "\n\t\r")
        )
...
class ScenarioDatabaseDialog(QtWidgets.QDialog):
    ...
    @classmethod
    def construct_dialog(
        cls, parent: QtWidgets.QWidget = None, options: list = None
...
class LocationLinkingDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.use_alternatives_checkbox = QtWidgets.QCheckBox(
            "Use generic alternatives (RoW, GLO) as fallback"
        )
        self.use_alternatives_checkbox.setToolTip(
            "If the location is not found, try to match to generic locations "

...
class UncertaintyWizard(QtWidgets.QWizard):
    ...
    def update_uncertainty(self):
        ...
        if self.obj.data_type == "exchange":
            signals.exchange_uncertainty_modified.emit(
                self.obj.data, self.uncertainty_info
            )
            if self.using_pedigree:
                signals.exchange_pedigree_modified.emit(
                    self.obj.data, self.pedigree.matrix.factors
                )
        elif self.obj.data_type == "parameter":
            signals.parameter_uncertainty_modified.emit(
                self.obj.data, self.uncertainty_info
            )
            if self.using_pedigree:
                signals.parameter_pedigree_modified.emit(
                    self.obj.data, self.pedigree.matrix.factors
                )

...
class Signals(QObject):
    ...
    delete_database_confirmed = Signal(
        str
    )  # This database has been deleted | name of database
    copy_database = Signal(
        str
    )  # Copy this database | name of the database to be copied
    ...
    databases_changed = (
        Signal()
    )  # The list of databases in this project has changed e.g. a database was added
    database_changed = Signal(
        str
    )  # This database has changed e.g. an activity was added to it | name of database
    ...
    new_activity = Signal(
        str
    )  # Trigger dialog to create a new activity in this database | name of database
    ...
    duplicate_activity_new_loc = Signal(
        tuple
    )  # Trigger dialog to duplicate this activity to a new location | key of activity
    duplicate_activities = Signal(
        list
    )  # Duplicate these activities | list of activity keys
    duplicate_activity_to_db = Signal(
        str, object
    )  # Duplicate this activity to another database | name of target database, BW2 actiivty object
    # TODO write below 2 signals to work without the str, source database is already stored in activity keys
    duplicate_to_db_interface = Signal(
        tuple, str
    )  # Trigger dialog to duplicate actiivty to another database | key of activity, name of source database
    duplicate_to_db_interface_multiple = Signal(
        list, str
    )  # Trigger dialog to duplicate actiivty to another database | list of activity keys, name of source database
    ...
    activity_modified = Signal(
        tuple, str, object
    )  # This was changed about this activity | key of activity, name of the changed field, new content of the field
    relink_activity = Signal(
        tuple
    )  # Trigger dialog to relink this activity | key of activity

    # Exchanges
    exchanges_deleted = Signal(
        list
    )  # These exchanges should be deleted | list of exchange keys
    exchanges_add = Signal(
        list, tuple
    )  # Add these exchanges to this activity | list of exchange keys to be added, key of target activity
    exchanges_add_w_values = Signal(
        list, tuple, dict
    )  # Add these exchanges to this activity with these values| list of exchange keys to be added, key of target activity, values to add per exchange
    exchange_modified = Signal(
        object, str, object
    )  # This was changed about this exchange | exchange object, name of the changed field, new content of the field
    # Exchange object and uncertainty dictionary
    exchange_uncertainty_wizard = Signal(
        object
    )  # Trigger uncertainty dialog for this exchange | exchange object
    exchange_uncertainty_modified = Signal(
        object, dict
    )  # The uncertainty data for this exchange was modified | exchange object, uncertainty data
    exchange_pedigree_modified = Signal(
        object, object
    )  # The pedigree uncertainty data for this exchange was modified | exchange object, pedigree data

    # Parameters
    add_parameter = Signal(
        tuple
    )  # Trigger dialog to add parameter to this exchange | key of exchange
    add_activity_parameter = Signal(
        tuple
    )  # Add a parameter to this exchange | key of exchange
    add_activity_parameters = Signal(
        list
    )  # Add parameter to these exchanges | list of exchange keys
    ...
    rename_parameter = Signal(
        object, str
    )  # Trigger dialog to rename parameter | parameter object, type (project, database or activity)
    parameter_renamed = Signal(
        str, str, str
    )  # This parameter was renamed | old name, type (project, database or activity), new name
    exchange_formula_changed = Signal(
        tuple
    )  # The formula for an exchange in this activity was changed | key of activity
    parameter_modified = Signal(
        object, str, object
    )  # This parameter was modified | parameter object, name of the changed field, new content of the field
    parameter_uncertainty_modified = Signal(
        object, dict
    )  # The uncertainty data for this parameter was modified | parameter object, uncertainty data
    parameter_pedigree_modified = Signal(
        object, object
    )  # The pedigree uncertainty data for this parameter was modified | parameter object, pedigree data
    delete_parameter = Signal(
        object
    )  # Delete this parameter | proxy index of the table view of the parameter
    ...
    clear_activity_parameter = Signal(
        str, str, str
    )  # Clear this parameter | name of database, name of code, name of group

    # Calculation Setups
    new_calculation_setup = Signal()  # Trigger dialog for a new calculation setup
    copy_calculation_setup = Signal(
        str
    )  # Trigger dialog for copying this calculation setup | name of calculation setup
    delete_calculation_setup = Signal(
        str
    )  # Delete this calculation setup | name of calculation setup
    rename_calculation_setup = Signal(
        str
    )  # Trigger dialog to rename this calculation setup | name of calculation setup
    ...
    copy_method = Signal(
        tuple, str
    )  # Copy this method | tuple of impact category, level of tree OR the proxy
    delete_method = Signal(
        tuple, str
    )  # Delete this method | tuple of impact category, level of tree OR the proxy
    edit_method_cf = Signal(
        tuple, tuple
    )  # Edit this CF for this method | tuple of characterization factor, tuple of method
    remove_cf_uncertainties = Signal(
        list, tuple
    )  # Remove uncertainty data for these CFs | list of CFs to remove info from, tuple of method
    ...
    set_uncertainty = Signal(
        tuple
    )  # Set uncertainty of CF | proxy index of the table view of the CF
    add_cf_method = Signal(
        tuple, tuple
    )  # Add uncertainty to CF | tuple of CF, tuple impact category
    delete_cf_method = Signal(
        tuple, tuple
    )  # Delete this CF from impact category | tuple of CF, tuple of impact category
